class GitGraphView {
	...
	public loadCommits(commits: GG.GitCommitNode[], commitHead: string | null, moreAvailable: boolean, hard: boolean) {
		if (!hard && this.moreCommitsAvailable === moreAvailable && this.commitHead === commitHead && arraysEqual(this.commits, commits, (a: GG.GitCommitNode, b: GG.GitCommitNode) => a.hash === b.hash && arraysEqual(a.refs, b.refs, (a: GG.GitRef, b: GG.GitRef) => a.name === b.name && a.type === b.type) && arraysEqual(a.parentHashes, b.parentHashes, (a: string, b: string) => a === b))) {
		...
		addListenerToClass('commit', 'contextmenu', (e: Event) => {
			...
			showContextMenu(<MouseEvent>e, [
				...
				},
				{
					...
					onClick: function(this: GitGraphView) {
						...
						if (this.commits[this.commitLookup[hash]].parentHashes.length === 1) {
							...
							let options = this.commits[this.commitLookup[hash]].parentHashes.map((parentHash: string, index: number) => ({
								name: abbrevCommit(parentHash) + (typeof this.commitLookup[parentHash] === 'number' ? ': ' + this.commits[this.commitLookup[parentHash]].message : ''),
								value: (index + 1).toString()
							}));
							showSelectDialog('Are you sure you want to cherry pick merge commit <b><i>' + abbrevCommit(hash) + '</i></b>? Choose the parent hash on the main branch, to cherry pick the commit relative to:', '1', options, 'Yes, cherry pick commit', (parentIndex: string) => {
				...
				},
				{
					...
					onClick: () => {
						if (this.commits[this.commitLookup[hash]].parentHashes.length === 1) {
							...
							showSelectDialog('Are you sure you want to revert merge commit <b><i>' + abbrevCommit(hash) + '</i></b>? Choose the parent hash on the main branch, to revert the commit relative to:', '1', options, 'Yes, revert commit', (parentIndex: string) => {
